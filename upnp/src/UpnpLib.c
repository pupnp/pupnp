...
int UpnpLib_assign(UpnpLib *p, const UpnpLib *q)
{
	...
	if (p != q) {
		ok = ok && UpnpLib_set_virtualDirCallback(
				   p, UpnpLib_get_virtualDirCallback(q));
		ok = ok && UpnpLib_set_pVirtualDirList(
				   p, UpnpLib_get_pVirtualDirList(q));
		ok = ok && UpnpLib_set_GlobalClientSubscribeMutex(p,
				   UpnpLib_get_GlobalClientSubscribeMutex(q));
		ok = ok && UpnpLib_set_GlobalHndRWLock(
				   p, UpnpLib_get_GlobalHndRWLock(q));
		ok = ok && UpnpLib_set_gUUIDMutex(p, UpnpLib_get_gUUIDMutex(q));
		ok = ok &&
		     UpnpLib_set_gSDKInitMutex(p, UpnpLib_get_gSDKInitMutex(q));
		ok = ok &&
		     UpnpLib_set_gTimerThread(p, UpnpLib_get_gTimerThread(q));
		ok = ok && UpnpLib_set_gSendThreadPool(
				   p, UpnpLib_get_gSendThreadPool(q));
		ok = ok && UpnpLib_set_gRecvThreadPool(
				   p, UpnpLib_get_gRecvThreadPool(q));
		ok = ok && UpnpLib_set_gMiniServerThreadPool(
				   p, UpnpLib_get_gMiniServerThreadPool(q));
		ok = ok && UpnpLib_set_bWebServerState(
				   p, UpnpLib_get_bWebServerState(q));
		ok = ok && UpnpLib_set_webCallback_HostValidate(
				   p, UpnpLib_get_webCallback_HostValidate(q));
		ok = ok &&
		     UpnpLib_set_webCallback_HostValidateCookie(
			     p, UpnpLib_get_webCallback_HostValidateCookie(q));
		ok = ok && UpnpLib_set_allowLiteralHostRedirection(p,
				   UpnpLib_get_allowLiteralHostRedirection(q));
		ok = ok && UpnpLib_set_gIF_NAME(p, UpnpLib_get_gIF_NAME(q));
		ok = ok && UpnpLib_set_gIF_IPV4(p, UpnpLib_get_gIF_IPV4(q));
		ok = ok && UpnpLib_set_gIF_IPV4_NETMASK(
				   p, UpnpLib_get_gIF_IPV4_NETMASK(q));
		ok = ok && UpnpLib_set_gIF_IPV6(p, UpnpLib_get_gIF_IPV6(q));
		ok = ok && UpnpLib_set_gIF_IPV6_PREFIX_LENGTH(
				   p, UpnpLib_get_gIF_IPV6_PREFIX_LENGTH(q));
		ok = ok && UpnpLib_set_gIF_IPV6_ULA_GUA(
				   p, UpnpLib_get_gIF_IPV6_ULA_GUA(q));
		ok = ok &&
		     UpnpLib_set_gIF_IPV6_ULA_GUA_PREFIX_LENGTH(
			     p, UpnpLib_get_gIF_IPV6_ULA_GUA_PREFIX_LENGTH(q));
		ok = ok && UpnpLib_set_gIF_INDEX(p, UpnpLib_get_gIF_INDEX(q));
		ok = ok &&
		     UpnpLib_set_LOCAL_PORT_V4(p, UpnpLib_get_LOCAL_PORT_V4(q));
		ok = ok &&
		     UpnpLib_set_LOCAL_PORT_V6(p, UpnpLib_get_LOCAL_PORT_V6(q));
		ok = ok && UpnpLib_set_LOCAL_PORT_V6_ULA_GUA(
				   p, UpnpLib_get_LOCAL_PORT_V6_ULA_GUA(q));
		ok = ok &&
		     UpnpLib_set_HandleTable(p, UpnpLib_get_HandleTable(q));
		ok = ok && UpnpLib_set_gMediaTypeArray(
