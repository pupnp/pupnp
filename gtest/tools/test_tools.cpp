...
TEST(ToolsTestSuite, initializing_interface_addresses)
{
	...
	EXPECT_EQ(ifaddr->ifa_flags,
		(const unsigned int)0 | IFF_LOOPBACK | IFF_UP);
	...
	EXPECT_EQ(
		ifa_netmask_in->sin_addr.s_addr, (const unsigned int)15794175);
	EXPECT_EQ(ifaddr->ifa_flags,
		(const unsigned int)0 | IFF_UP | IFF_BROADCAST | IFF_MULTICAST);
	char broadcast_address[INET_ADDRSTRLEN];
	inet_ntop(AF_INET,
		&ifa_ifu_in->sin_addr.s_addr,
		broadcast_address,
		INET_ADDRSTRLEN);
	...
	EXPECT_EQ(ifa_netmask_in->sin_addr.s_addr,
