...
// for mocked network interfaces.
{
	...
	struct sockaddr_in
		ifa_ifu; // broadcast addr or point-to-point dest addr
	...
	// are set to an ipv4 UP interface, supporting broadcast and multicast.
	// Returns true if successful.
	{
		...
		// bitmask << "', netmask: "
		//           << netmasks[std::stoi(bitmask)] << ", slashpos: "
		//           << slashpos << "\n";
		...
		// subnet ) broadcast = ip-addr or the inverted subnet-mask
		ifa_ifu.sin_addr.s_addr =
			ifa_addr.sin_addr.s_addr | ~ifa_netmask.sin_addr.s_addr;
...
// to call capture(..) again.
{
	...
	void capture(int fd)
	{
		...
		mFd_log =
			::open(mCaptFname, O_WRONLY | O_CREAT | O_TRUNC, 0660);
		if (mFd_log < 0) {
			::close(mFd_old);
			return;
		}
